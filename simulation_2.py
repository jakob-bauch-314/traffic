...
for _ in range(0, crossings_amount):
    crossings.append(header.Crossing((random.random()*WIDTH - shift_x)/unit_size, (random.random()*HEIGHT - shift_y)/unit_size, 0.0))

streets = []
for i in range(0, crossings_amount):
    # all possible streets sorted by length
    all_streets = sorted(list(map(lambda j: header.Street(i, j, math.sqrt(math.pow(crossings[j].x - crossings[i].x, 2) + math.pow(crossings[j].y - crossings[i].y, 2)), 1), range(0, crossings_amount))), key = lambda street: street.length)
    # add to streets
